...
// POST /api/bets/scan/text  { text: "<shared bet text>" }
// Parse a pasted/shared bet (e.g. the text from a bet365 "share bet" link, or
// any bookmaker's confirmation text) into the app's bet shape. Uses the same
// extractor and free-scan quota as the image scanner.
router.post('/text', async (req, res) => {
  if (!config.anthropic.apiKey) {
    return res.status(503).json({
      error: 'Bet reading isn’t enabled on this server. Set ANTHROPIC_API_KEY to turn it on.',
    });
  }
  ...
  if (!canScan(req.userId)) {
    const e = entitlements(req.userId);
    return res.status(402).json({
      error: `You've used all ${e.scans.limit} free reads this month. Upgrade to Pro for unlimited.`,
      upgrade: true,
      scans: e.scans,
    });
  }
  ...
  if (!text) return res.status(400).json({ error: 'Paste the bet text first.' });
  if (text.length > 6000) {
    return res.status(413).json({ error: 'That’s a lot of text — paste just the bet, not the whole page.' });
  }
  ...
  try {
          ...
          content: [
            {
              type: 'text',
              text:
                'Extract the bet from this shared bet text (copied from a bookmaker app such as bet365). ' +
                'It may contain the selections and combined odds but often NOT the stake — leave stake 0 if it is not shown.\n\n' +
                text,
            },
          ],
        },
    ...
    if (message.stop_reason === 'refusal') {
      return res.status(422).json({ error: 'Couldn’t read a bet from that text.' });
    }
    ...
    if (!parsed) {
      return res.status(422).json({ error: 'Couldn’t read a bet from that text. Try adding it manually.' });
    }
    ...
    console.error('[scan/text] extraction failed:', err?.message || err);
    res.status(502).json({ error: 'Bet reading is temporarily unavailable. Please add the bet manually.' });
  }
});
